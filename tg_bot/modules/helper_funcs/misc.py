...
# PTB 20+ message length
try:
    from telegram.constants import MessageLimit
    MAX_MESSAGE_LENGTH = getattr(MessageLimit, "MAX_TEXT_LENGTH", 4096)
except Exception:
...
def build_keyboard(buttons):
    keyb = []
    for btn in buttons:
        if getattr(btn, "same_line", False) and keyb:
...
def revert_buttons(buttons):
    return "".join(
        ...
        if getattr(btn, "same_line", False)
...
def upload_text(data: str) -> typing.Optional[str]:
    ...
    paste_meta = [
        [base64.b64encode(cipher.nonce).decode(), base64.b64encode(salt).decode(), 100000, 256, 128, "aes", "gcm", "zlib"],
        "syntaxhighlighting",
        0,
        0,
    ]
    ...
    resp = requests.post(
        ...
        data=json.dumps({"v": 2, "adata": paste_meta, "ct": base64.b64encode(ct + tag).decode(), "meta": {"expire": "1week"}}, separators=(",", ":")),
    )
    data = resp.json()
