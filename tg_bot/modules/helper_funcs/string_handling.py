...
def markdown_parser(
    ...
    for ent, ent_text in entities.items():
        ...
        # entity type can be str (old) or MessageEntityType (new)
        ent_type = ent.type if isinstance(ent.type, str) else getattr(ent.type, "value", str(ent.type))

        # we only care about code, url, text links
        if ent_type in ("code", "url", "text_link"):
            ...
            if ent_type == "url":
                if any(match.start(1) <= start and end <= match.end(1) for match in LINK_REGEX.finditer(txt)):
                    # Inside a markdown link [text](url) already
                    continue
                else:
                    # Avoid URL mangling by escaping as legacy Markdown (version=1)
                    res += _selective_escape(txt[prev:start] or "") + escape_markdown(ent_text, version=1)
...
def extract_time(message, time_val):
    """
    Returns a unix timestamp or "" on error.
    Sends an error reply to the user. Compatible with PTB 20+ (async) and legacy sync usage:
    - If reply_text is awaitable, it will be scheduled on the running loop.
    """
    def _notify(msg_text: str):
        try:
            res = message.reply_text(msg_text)
            if inspect.isawaitable(res):
                try:
                    loop = asyncio.get_running_loop()
                    loop.create_task(res)
                except RuntimeError:
                    # No running loop; best-effort blocking run (not typical inside PTB handlers)
                    asyncio.run(res)
        except Exception:
            pass
    ...
    if any(time_val.endswith(unit) for unit in ("m", "h", "d")):
        ...
        if not time_num.isdigit():
            _notify("Invalid time amount specified.")
            return ""
        ...
        _notify("Invalid time type specified. Expected m,h, or d, got: {}".format(time_val[-1]))
        return ""
