...
async def chatmemberupdates(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[str]:
    ...
    message = update.effective_message  # may be None for chat_member updates
    ...
    status_change, title_change = extract_status_change(update.chat_member)

    # Ignore actions performed by the bot itself; handled elsewhere
    if update.chat_member.from_user and str(update.chat_member.from_user.id) == str(bot.id):
        return ""
    ...
    if status_change is not None:  # extract chat changes
        oldstat, newstat = status_change
        oldstat = str(oldstat)
        newstat = str(newstat)
        ...
        # Generic ban/unban/join/leave cases
        ...
        if oldstat in ("left", "kicked") and newstat == "member":
        ...
        if oldstat in ("member", "administrator") and newstat == "left":
            if member_name == cause_name:
