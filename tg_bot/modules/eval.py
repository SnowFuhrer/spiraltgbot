...
async def send(msg, bot, update):
    if msg is None:
        return
    if len(str(msg)) > 2000:
        with io.BytesIO(str(msg).encode()) as out_file:
...
@kigcmd(command=("e", "ev", "eva", "eval"), filters=filters.User(user_id=SYS_ADMIN))
@rate_limit(40, 60)
...
@kigcmd(command=("x", "ex", "exe", "exec", "py"), filters=filters.User(user_id=SYS_ADMIN))
@rate_limit(40, 60)
...
@kigcmd(command="clearlocals", filters=filters.User(user_id=SYS_ADMIN))
@rate_limit(40, 60)
