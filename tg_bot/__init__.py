...
# Quiet noisy HTTP logs from httpx/httpcore
...
class KigyoINIT:
    def __init__(self, parser: ConfigParser):
        ...
        self.STANZA_REFRESH_TOKEN: str = self.parser.get("STANZA_REFRESH_TOKEN", None)
        self.bot_id: int = 0
        ...
        # Defaults to cloud; can be overridden by env at runtime below
        ...
        self.POSTGRES_MAX_OVERFLOW: int = self.parser.getint("POSTGRES_MAX_OVERFLOW", 10)
        self.POSTGRES_POOL_TIMEOUT: int = self.parser.getint("POSTGRES_POOL_TIMEOUT", 30)
        self.POSTGRES_POOL_RECYCLE: int = self.parser.getint("POSTGRES_POOL_RECYCLE", 1800)
...
STANZA_REFRESH_TOKEN = KInit.STANZA_REFRESH_TOKEN
...
# Build Bot and Application for PTB 20+
# Use HTTPXRequest to control timeouts
_httpx_request = HTTPXRequest(read_timeout=10, connect_timeout=10)

# Resolve Bot API base URLs (env overrides config)
env_base = os.getenv("BOT_API_URL", "").strip()
if env_base:
    base = env_base.rstrip("/")
    base_url = f"{base}/bot"
    base_file_url = f"{base}/file/bot"
else:
    base_url = KInit.BOT_API_URL.rstrip("/")
    base_file_url = KInit.BOT_API_FILE_URL.rstrip("/")
...
builder = ApplicationBuilder().bot(_bot)

# Optional: force HTTP/1.1 for some local servers
if HTTPVersion and os.getenv("BOT_API_FORCE_HTTP1", "").lower() in ("1", "true", "yes"):
    builder = builder.http_version(HTTPVersion.HTTP_1_1).get_updates_http_version(HTTPVersion.HTTP_1_1)

application = builder.build()

# Backwards-compat alias so modules can still do `from tg_bot import dispatcher`
dispatcher = application

# Load at end to ensure all prev variables have been set
# NOTE: CustomCommandHandler must be ported to PTB 20+ before re-enabling.
# from tg_bot.modules.helper_funcs.handlers import CustomCommandHandler
# if CUSTOM_CMD and len(CUSTOM_CMD) >= 1:
#     # Only re-enable after updating CustomCommandHandler to PTB 20+
#     pass
