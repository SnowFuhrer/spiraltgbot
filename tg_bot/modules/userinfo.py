...
# PTB 20+: no MAX_MESSAGE_LENGTH export. Define it.
try:
    from telegram.constants import MessageLimit
    MAX_MESSAGE_LENGTH = getattr(MessageLimit, "TEXT_LENGTH", 4096)
except Exception:
...
async def about_bio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.reply_to_message:
        repl_message = message.reply_to_message
        target_user_id = repl_message.from_user.id
        ...
        if len(bio) == 2:
            if len(bio[1]) < MAX_MESSAGE_LENGTH // 4:
                sql.set_user_bio(target_user_id, bio[1])
                await message.reply_text(
