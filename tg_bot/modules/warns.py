...
async def warn(
    ...
    # PTB 20+: is_user_admin is async in your codebase
    if await is_user_admin(update, user.id):
        return

    if user.id in SARDEGNA_USERS:
        if warner:
            await message.reply_text("SARDEGNAs cant be warned.")
        else:
    ...
    if user.id in WHITELIST_USERS:
        if warner:
            await message.reply_text("Whitelisted users are warn immune.")
        else:
            await message.reply_text(
    ...
    if num_warns >= limit:
        ...
        if soft_warn:
            # Soft action = kick (ban + unban pattern)
...
async def warn_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if user_id:
        if message.reply_to_message and message.reply_to_message.from_user.id == user_id:
            ...
            return await warn(member.user, update, reason, message, warner)
    else:
...
async def add_warn_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # We use a single catch-all handler (WARN_FILTER_HANDLER) that checks DB,
    # so no need to manage per-keyword handlers here on PTB 20+.
...
async def list_warn_filters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat: Optional[Chat] = update.effective_chat
    triggers = sql.get_chat_warn_triggers(chat.id)
    if not triggers:
        await update.effective_message.reply_text("No warning filters are active here!")
        return
    text = CURRENT_WARNING_FILTER_STRING + "\n".join(
        f" - <code>{html.escape(t)}</code>" for t in sorted(triggers)
    )
