...
async def join_fed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) >= 1:
        ...
        await message.reply_text("This chat has joined the federation: {}!".format(getfed["fname"]))
...
async def leave_fed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    getuser = member.status
    if str(getuser) == "creator" or user.id in SUDO_USERS:
...
async def fed_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(members) == 0:
        ...
        text += "\n🔱 Admin:\n"
        for x in members:
            u = await context.bot.get_chat(x)
            name = u.first_name or "Deleted"
            text += " • {}\n".format(mention_html(u.id, name))
...
@rate_limit(40, 60)
async def fed_ban(update: Update, context: ContextTypes.DEFAULT_TYPE):  # sourcery no-metrics
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    args = context.args
        ...
        for fedschat in fed_chats:
            try:
                ...
                if excp.message in FBAN_ERRORS:
                    try:
                        await context.bot.get_chat(fedschat)
...
async def unfban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for fedchats in chat_list:
        ...
        try:
            member = await context.bot.get_chat_member(fedchats, user_id)
            if str(member.status) == "kicked":
                await context.bot.unban_chat_member(fedchats, user_id)
        except BadRequest as excp:
    ...
    if len(subscriber) != 0:
        for fedsid in subscriber:
            ...
            for fedschat in all_fedschat:
                try:
                    await context.bot.unban_chat_member(fedschat, user_id)
                except BadRequest as excp:
                    if excp.message in FBAN_ERRORS:
                        try:
                            await context.bot.get_chat(fedschat)
...
async def fed_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        text += "\n\n- {}".format(mention_markdown(user.id, broadcaster, version=1))
        chat_list = sql.all_fed_chats(fed_id)
        failed = 0
        for chat_id in chat_list:
            title = "*New broadcast from Fed {}*\n".format(fedinfo["fname"])
            try:
                await context.bot.send_message(chat_id, title + text, parse_mode="markdown")
            except TelegramError:
                try:
                    ...
                    log.info(
                        "Chat {} has leave fed {} because I was kicked".format(
                            chat_id, fedinfo["fname"]
                        )
                    )
                    continue
                failed += 1
                log.warning("Couldn't send broadcast to {}".format(str(chat_id)))
...
@rate_limit(40, 60)
async def fed_ban_list(update: Update, context: ContextTypes.DEFAULT_TYPE):  # sourcery no-metrics
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    args = context.args
    ...
    if args:
        if args[0] == "json":
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(backups.encode()) as output:
    ...
    try:
        ...
        with BytesIO(cleantext.encode()) as output:
...
async def fed_chats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for chats in getlist:
        try:
            chat_name = (await context.bot.get_chat(chats)).title
        except (Unauthorized, Forbidden):
...
@rate_limit(40, 60)
async def fed_import_bans(update: Update, context: ContextTypes.DEFAULT_TYPE):  # sourcery no-metrics
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    msg = update.effective_message
    chat_data = context.chat_data
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if fileformat == "json":
            ...
            filename = "fban_{}.csv".format(msg.reply_to_message.document.file_id)
            await file_info.download_to_drive(custom_path=filename)

            with open(filename, "r", encoding="utf8") as csvFile:
                reader = csv.reader(csvFile)
                for data in reader:
                    try:
                        import_userid = int(data[0])
...
@rate_limit(40, 60)
async def fed_stat_user(update: Update, context: ContextTypes.DEFAULT_TYPE):  # sourcery no-metrics
    user = update.effective_user  # type: Optional[User]
    msg = update.effective_message
    args = context.args
...
async def subs_feds(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not fed_id:
        await send_message(update.effective_message, "This chat is not in any federation!")
        return
...
async def get_myfeds_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_message(msg, text, parse_mode="markdown")
